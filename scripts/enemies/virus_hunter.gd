...
@export var decoy_count: int = 2           # التحكم بعدد النسخ الوهمية
@export var decoy_spawn_offset: float = 60.0 # التحكم بمسافة ابتعاد النسخ يميناً ويساراً
...
# --- تسلسل التيلبورت وتشغيل الصوت وإنشاء النسخ ---
func _start_teleport_sequence() -> void:
	...
	# 2. إنشاء النسخ بحسب العدد والمسافة المحددين
	...
	# 3. الانتقال للموقع الجديد مع التأكد من وجود أرضية تحته
...
# --- إنشاء نسخة وهمية شفافة بـ 1 HP وبدون تصادم مع اللاعب ---
func _spawn_decoy(spawn_pos: Vector2) -> void:
	...
	decoy.collision_mask = 1 # التصادم مع الأرضية فقط

	# منع أي تصادم أو دفع للاعب
...
var lifetime: float = 4.0 # تختفي تلقائياً بعد 4 ثواني
var _target: Node2D
...
func _fake_fire() -> void:
	...
	if projectile_scene:
		...
		proj.modulate = Color(1.0, 1.0, 1.0, 0.15) # شفافية المقذوف (15%)
		if "damage" in proj:
			proj.damage = 0
...
"""
	...
	if visual and visual is AnimatedSprite2D:
		...
		decoy_sprite.play()
		decoy_sprite.modulate = Color(1.0, 1.0, 1.0, 0.25) # شفافية النسخة (25%)
