...
func _ready() -> void:
	# Binds the triggers to your specified TextureRects inside Platform8
	$TRIGGER1.body_entered.connect(_on_player_touched.bind($TextureRect8))
	...
	$TRIGGER4.body_entered.connect(_on_player_touched.bind($TextureRect))
